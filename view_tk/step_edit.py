...
class TVStepEditor(Frame):
    ...
    def init_treeview(self, parent):
        ...
        self.tv["columns"] = (
            [f'#{i+1}' for i in range(self.page_size)]
        )
    ...
    def init_toolbar(self, parent):
        ...
        self.scale = Scale(
            bf, from_=0., to=100., orient=HORIZONTAL, value=50.,
        )
    ...
    def connect_callbacks(self):
        self.tv.bind(
            "<MouseWheel>",
            lambda event: self.tv.yview("scroll", -event.delta, "units")
        )
        self.tv.bind(
            "<Shift-MouseWheel>",
        ...
        self.tv.bind(
            '<Command-w>',
            lambda e: self.tool_widgets['Wrap'].invoke()
        )
        self.tv.bind(
            '<Command-r>',
            lambda e: self.tool_widgets['Raw'].invoke()
        )
    ...
    def populate_layers(self):
        def insert_layer_item(item, **kwargs):
            ...
            if 'par' in item:
                ...
                blk = 'Trg'
            self.tv.insert(
                item[:6], 'end', item, text=name, tag=blk.lower(),
            )
    ...
    def populate_pages(self):
        def insert_page_item(item, **kwargs):
            ...
            self.tv.insert(
                ...
                item,
                text=text,
                tag='even' if pg % 2 else 'odd'
            )
        self.buf.for_all_pars(self.buf.for_all_pages,
                          row_func=insert_page_item)  # , force=True)
        self.buf.for_all_trgs(self.buf.for_all_pages,
                          row_func=insert_page_item)  # , force=True)
    ...
    def show_all_pages(self):
        self.buf.for_all_pars(
            self.buf.for_all_pages, row_func=self.show_row)
    ...
    def show_row(self, item, offset=0):
        ...
        if not self.raw.get():
            if blk == 'par':
                values = [
                    self.buf.par_layer_types[l]['str'](v) for v in values
                ]
            else:
