...
#  This code copied from pygubu and following this example:
#  https://github.com/alejandroautalan/pygubu/issues/73
#
#  with a few fixes and cosmetics applied.
...
class EditableTreeview(Treeview):
    ...
    def __init__(self, parent=None, **kw):
        Treeview.__init__(self, parent, **kw)
    ...
    def yview(self, *args):
        ...
        self.after_idle(self.__updateWnds)
        Treeview.yview(self, *args)

    def yview_scroll(self, number, what):
        self.after_idle(self.__updateWnds)
        Treeview.yview_scroll(self, number, what)

    def yview_moveto(self, fraction):
        self.after_idle(self.__updateWnds)
        Treeview.yview_moveto(self, fraction)

    def xview(self, *args):
        """Update inplace widgets position when doing horizontal scroll"""
        self.after_idle(self.__updateWnds)
        Treeview.xview(self, *args)

    def xview_scroll(self, number, what):
        self.after_idle(self.__updateWnds)
        Treeview.xview_scroll(self, number, what)

    def xview_moveto(self, fraction):
        self.after_idle(self.__updateWnds)
        Treeview.xview_moveto(self, fraction)
    ...
    def inplace_checkbutton(
        ...
        svar.set(self.__get_value(col, item))
        if col not in self._inplace_widgets:
            self._inplace_widgets[col] = Checkbutton(
                self,
                textvariable=svar,
                variable=svar,
                onvalue=onvalue,
...
if __name__ == "__main__":
    ...
    cols = ("Index", "Value", "Price", "LAST_PRICE")
    etv["columns"] = cols
    for col in cols:
