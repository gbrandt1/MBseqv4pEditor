...
class TrackEditor:
    ...
    def set_sdcard(self, sdcard=None):
        ...
        self.cbx_track_preset.configure(values=list(sdcard.track_presets.keys()))
    ...
    def set_track(self, track):
        ...
        self.svar["layer_config"] = StringVar(value=list(TRACK_LAYER_CONFIG.keys())[0])
    ...
    def init_fx_editors(self, parent):
        ...
        row3 = self.init_row(frm_tab, ("FX: Duplicate",))
    ...
    def init_page(self, parent, page):
        ...
        for txt, var in TRACK_GUI_PAGES[page]:
            ...
            ent.bind("<FocusOut>", lambda e, var=var: self.update_track_attr(var))
    ...
    def init_pattern_info(self, parent):
        ...
        Label(frm_pattern, textvariable=self.svar["pattern_name"]).pack(side=LEFT)
        return frm_pattern

    def init_track_config_editor(self, parent):
        frm_tab = LabelFrame(parent, text="Track Configuration", padding=PAD_LF)
        ...
        self.cbx_mode.bind(
            "<FocusOut>", lambda e: self.do_configure_track("event_mode")
        )
        ...
        def ent_cfg(parent, text, var, row, col):
            ...
            ent.bind("<FocusOut>", lambda e, var=var: self.do_configure_track(var))
        ...
        btn_trkcfg = Button(frm_cfg, text="Configure", command=self.do_configure_track)
        btn_trkcfg.pack()
        return frm_tab
    ...
    def init_drum_name_editor(self, parent):
        ...
        for d in range(16):
            ...
            ent_name.bind("<FocusOut>", lambda e: self.update_track_attr("name"))
    ...
    def reset_track(self):
        ...
        yes = askokcancel("Reset Track", f"Reset everything in track to defaults?")
        if yes:
            self.track.reset()
            self.track.modified()
