...
class SongStep:  # Observable):
    ...
    def _set(
        self, action=0, value=0, npn1="-:--", npn2="-:--", npn3="-:--", npn4="-:--"
    ):
        self.action = action
    ...
    def __repr__(self):
        step = self.str_action() + self.str_action_value() + "".join(self.str_groups())
        return step
...
class Song(Observable):
    def __init__(self, parent, content=None):
        super().__init__(parent)
        if content:
            (self.name, self.guide, dummy) = unpack("18sBB", content[:20])
        else:
            self.clear()
        # .replace(' ','\uFE4D') #'\u00B7')
...
class SongBank(Observable):
    def __init__(self, parent, filename):
        ...
        (self.header, self.name, self.num_songs, self.song_size) = unpack(
