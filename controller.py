...
class Controller(Observable):
    ...
    def set_pattern_selection(self, selection):
        ...
        if not isinstance(selection, list):
            logger.error(
                f'set_pattern_selection: Selection is not list: {selection}')
        self.pattern_selection = selection
        #  for npn in selection:
        #     self.pattern_selection[npn] = selection[npn]
        logger.debug(
            f'set_pattern_selection: Set selection: {self.pattern_selection}')
    ...
    def set_track(self, nt=0):
        ...
        if self.track != self.tracks[nt]:
            ...
            # if nb != nt//4:
            logger.info(
                f'Set active track {nt} ({self.group}, {nt%4}): '
                f'{self.track}'
            )
            # self.show_status()
    ...
    def _paste_one_track(self, nt, track):
        ...
        # dump(new_track)
        logger.info(
            f'_paste_one_track to {nt} {new_track} {npn} {pattern}'
        )
