...
class Pattern(Observable):
    ...
    def __init__(self, parent, content):
        ...
        if (self.num_tracks != MAX_NUM_TRACKS):
            raise SeqV4PError(
                f'Pattern does not have {MAX_NUM_TRACKS} tracks:'
                f' {self.num_tracks}'
            )
        i = 24
...
class PatternBank(Observable):
    ...
    def __init__(self, parent, filename):
        ...
        if self.header != b'MBSEQV4_B\x00':
            raise SeqV4PError(
                f'Corrupt header {self.header} in {self.fullname}'
            )
        if self.pattern_size != 6008:
            raise SeqV4PError(
                f'Pattern reserved size {self.pattern_size} not 6008b'
            )
        # if self.name[19] != :
        ...
        if len(self.name) != 20:
            raise SeqV4PError(
                f'Pattern name "{self.name}" wrong size {len(self.name)}'
            )
        self.patterns = []
