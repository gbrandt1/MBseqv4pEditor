...
class Track(Observable):
    ...
    def __init__(self, parent, content):  # =None, pattern_name=None):
        ...
        (self.par, ) = unpack(
            f'{self.p_tot_size}s',
            content[216:216+self.p_tot_size]
        )
        (self.trg, ) = unpack(
            f'{self.t_tot_size}s',
            content[216+self.p_tot_size:216 +
                    self.p_tot_size+self.t_tot_size]
        )
    ...
    def par_set(self, step, par_layer, par_instrument, value):
        ...
        step_ix = (par_instrument*self.num_p_layers *
                   self.num_par_steps) + (
                       par_layer * self.num_par_steps) + step
    ...
    def par_get(self, step, par_layer, par_instrument):
        ...
        step %= self.num_par_steps
        step_ix = (par_instrument*self.num_p_layers *
                   self.num_par_steps) + (par_layer * self.num_par_steps) + step
    ...
    def trg_set(self, step, trg_layer, trg_instrument, value):
        '''Set trigger value.'''
        step_ix = (
            trg_instrument * self.num_t_layers *
            self.num_trg_steps//8
        ) + (trg_layer * self.num_trg_steps//8) + step//8
        if step_ix>=256:
            logger.info(
    ...
    def trg_get(self, step, trg_layer, trg_instrument):
        '''Get trigger value.'''
        #step %= self.num_par_steps
        step_ix = (trg_instrument*self.num_t_layers *
                   self.num_trg_steps//8) + (
                       trg_layer * self.num_trg_steps//8) + step//8
        step_mask = 1 << (step & 0x7)
    ...
    def get_par_layer_types(self):
        ...
        if self.event_mode == 'Drum':
            ...
            par_layer_types = [
                getattr(self, f'lay_const_a{l+1}')
                for l in range(self.num_p_layers)
            ]
