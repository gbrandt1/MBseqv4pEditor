...
EVENT_MODES = (
    "Note",  "Chord",  "CC" ,  "Drum", "Combined",
)

PLAY_MODES = (
  'Off', 'Normal', 'Transpose', 'Arpeggiator',
)

DIR_MODES = (
  'Forward', 'Backward',
  'PingPong','Pendulum',
  'Random_Dir','Random_Step','Random_D_S',
)

# From seq_ui_trkevent.c
TRACK_LAYER_CONFIG = OrderedDict()
  # mode    par_layers  par_steps  trg_layers  trg_steps  instruments
TRACK_LAYER_CONFIG["Note  (16*64)/(8*64) 1"  ] = ( 'Note',  16,  64, 8,  64,  1, )
TRACK_LAYER_CONFIG["Note  (8*128)/(8*128) 1"] =  ( 'Note',   8, 128, 8, 128,  1, )
TRACK_LAYER_CONFIG["Note  (4*256)/(8*256) 1"] =  ( 'Note',   4, 256, 8, 256,  1, )
TRACK_LAYER_CONFIG["Chord (16*64)/(8*64) 1"  ] = ( 'Chord', 16,  64, 8,  64,  1, )
TRACK_LAYER_CONFIG["Chord (8*128)/(8*128) 1"] =  ( 'Chord',  8, 128, 8, 128,  1, )
TRACK_LAYER_CONFIG["Chord (4*256)/(8*256) 1"] =  ( 'Chord',  4, 256, 8, 256,  1, )
TRACK_LAYER_CONFIG["CC    (16*64)/(8*64) 1"  ] = ( 'CC',    16,  64, 8,  64,  1, )
TRACK_LAYER_CONFIG["CC    (8*128)/(8*128) 1"] =  ( 'CC',     8, 128, 8, 128,  1, )
TRACK_LAYER_CONFIG["CC    (4*256)/(8*256) 1"] =  ( 'CC',     4, 256, 8, 256,  1, )
TRACK_LAYER_CONFIG["Drum  (4*16)/(2*64) 16"  ] = ( 'Drum',   4,  16, 2,  64, 16, )
#!
TRACK_LAYER_CONFIG["Drum  (4*16)/(2*128) 8" ] = ( 'Drum',   4,  16, 2, 128,  8, )
TRACK_LAYER_CONFIG["Drum  (4*16)/(1*256) 8" ] = ( 'Drum',   4,  16, 1, 256,  8, )
TRACK_LAYER_CONFIG["Drum  (1*64)/(2*64) 16"  ] = ( 'Drum',   1,  64, 2,  64, 16, )
TRACK_LAYER_CONFIG["Drum  (2*32)/(1*128) 16" ] = ( 'Drum',   2,  32, 1, 128, 16, )
TRACK_LAYER_CONFIG["Drum  (1*128)/(2*128) 8" ] = ( 'Drum',   1, 128, 2, 128,  8, )
TRACK_LAYER_CONFIG["Drum  (2*64)/(2*128) 8"  ] = ( 'Drum',   2,  64, 2, 128,  8, )
TRACK_LAYER_CONFIG["Drum  (2*64)/(1*256) 8"  ] = ( 'Drum',   2,  64, 1, 256,  8, )
TRACK_LAYER_CONFIG["Drum  (1*64)/(1*64) 16"  ] = ( 'Drum',   1,  64, 1,  64, 16, )
TRACK_LAYER_CONFIG["Drum  (1*128)/(1*128) 8" ] = ( 'Drum',   1, 128, 1, 128,  8, )
TRACK_LAYER_CONFIG["Drum  (1*256)/(1*256) 4" ] = ( 'Drum',   1, 256, 1, 256,  4, )

TRACK_PROP_IDX = {
    'attr': 0, 'index': 1, 'name': 2,
}
...
################################### not yet saved ################################################
    ( 'robotize_mask1'                  , 0x80,    0, '',), #ROBOTIZE_MASK1
    ( 'robotize_mask2'                  , 0x81,    0, '',), #ROBOTIZE_MASK2
    ( 'robotize_active'                 , 0x82,    0, '',), #ROBOTIZE_ACTIVE
    ( 'robotize_probability'            , 0x83,    0, '',), #ROBOTIZE_PROBABILITY
    ( 'robotize_skip_probability'       , 0x84,    0, '',), #ROBOTIZE_SKIP_PROBABILITY
    ( 'robotize_note'                   , 0x85,    0, '',), #ROBOTIZE_NOTE
    ( 'robotize_note_probability'       , 0x86,    0, '',), #ROBOTIZE_NOTE_PROBABILITY
    ( 'robotize_vel'                    , 0x87,    0, '',), #ROBOTIZE_VEL
    ( 'robotize_vel_probability'        , 0x88,    0, '',), #ROBOTIZE_VEL_PROBABILITY
    ( 'robotize_len'                    , 0x89,    0, '',), #ROBOTIZE_LEN
    ( 'robotize_len_probability'        , 0x8a,    0, '',), #ROBOTIZE_LEN_PROBABILITY
    ( 'robotize_oct'                    , 0x8b,    0, '',), #ROBOTIZE_OCT
    ( 'robotize_oct_probability'        , 0x8c,    0, '',), #ROBOTIZE_OCT_PROBABILITY
    ( 'robotize_sustain_probability'    , 0x8d,    0, '',), #ROBOTIZE_SUSTAIN_PROBABILITY
    ( 'robotize_nofx_probability'       , 0x8e,    0, '',), #ROBOTIZE_NOFX_PROBABILITY
    ( 'robotize_echo_probability'       , 0x8f,    0, '',), #ROBOTIZE_ECHO_PROBABILITY
    ( 'robotize_duplicate_probability'  , 0x90,    0, '',), #ROBOTIZE_DUPLICATE_PROBABILITY
